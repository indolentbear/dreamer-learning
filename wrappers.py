...
class Atari:
  ...
  def __init__(
    ...
    import gym
    version = 0 if sticky_actions else 4  # sticky_actions 严格行为
    ...
    self._buffers = [np.empty(shape, dtype=np.uint8) for _ in range(2)]   # 随机初始化两帧画面
...
class OneHotAction(object):
  ...
  def step(self, action, strict=False):
    ...
    reference[index] = 1
    if not np.allclose(reference, action) and strict:
